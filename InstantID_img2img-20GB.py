...
Scheduler: {scheduler}"""
    ...
    - If you're not satisfied with the similarity, try increasing the weight of "IdentityNet Strength" and "Adapter Strength, or try decreasing the weight of "img2img Strength" 
    - If you feel that the saturation is too high, first decrease the Adapter strength. If it remains too high, decrease the IdentityNet strength.
    ...
    with gr.Blocks(css=css) as gui:
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Row():
                    strength = gr.Slider(
                        label="img2img Denoising Strength (Lower values preserve more of the original image)",
                        minimum=0.1,
                        maximum=1.0,
                        step=0.05,
                        value=0.9,
                ...
                with gr.Accordion("⚙️ Style templates and other settings including custom resolution", open=False) as style_settings_accordion:
                    ...
                    with gr.Row():
                        enable_vae_tiling = gr.Checkbox(
                            ...
                            value=True,
                            info="Processes images in tiles to reduce VRAM usage during the final VAE decoding step without any quality loss. Best to enable only if you have 20GB VRAM or more."
                        )
                    with gr.Row():
