...
- You can use "InstantID_img2img.py" for pure img2img generation mode. I modified "instant-id-gradio-unlocked.py" to use the already existing "pipeline_stable_diffusion_xl_instantid_img2img.py" file. The InstantID_img2img-20GB file is optimized for 20GB+ VRAM cards, in fact even using InstantID_img2img.py uses much VRAM (can easily use more than 18GB). This img2img mode is just an experiment I made for myself (and it works good for the most part), therefore it's recommended to use the normal "instant-id-gradio-unlocked.py" which is how InstantID is intended to be used.
