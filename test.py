...
for _pat in _WARNING_PATTERNS:
    warnings.filterwarnings("ignore", message=_pat)

import logging
_logger = logging.getLogger("transformers.tokenization_utils_base")
_logger.addFilter(lambda r: "Token indices sequence length is longer" not in r.getMessage())

os.environ.update({
    "NO_ALBUMENTATIONS_UPDATE": "1",
    "TRANSFORMERS_OFFLINE": "1",
    "HF_HUB_CACHE": "models",
    "HF_HUB_CACHE_OFFLINE": "true",
    "GRADIO_ANALYTICS_ENABLED": "False",
    "GRADIO_DISABLE_TELEMETRY": "1",
})
...
def save_images(images, output_dir="output", generation_info=None, prefix=DEFAULT_FILE_PREFIX):
    ...
    used_numbers = [
        ...
        for f in existing
        if f[len(prefix):].split(".")[0].isdigit()
    ]
...
def get_depth_map(image):
    ...
    depth_map = torch.nn.functional.interpolate(
        depth_map.unsqueeze(1), size=(1024, 1024), mode="bicubic", align_corners=False,
    )
    ...
    if os.path.exists(models_dir):
        for folder in os.listdir(models_dir):
            if folder.startswith("models--"):
                ...
                if model_name.lower() not in EXCLUDED_MODELS_LOWER:
...
def get_available_loras():
    if not os.path.exists(LORAS_DIR):
        return []
    return [f for f in os.listdir(LORAS_DIR) if f.endswith(('.safetensors', '.ckpt', '.pt'))]
...
def _resolve_style_replacement(prompt_substitute):
    return " " if prompt_substitute == "Empty (none)" else prompt_substitute
...
def get_random_style_prompt(prompt_substitute="person"):
    ...
    replacement = _resolve_style_replacement(prompt_substitute)
    return style_prompt.replace("{prompt}", replacement).strip(), style_neg_prompt, DEFAULT_STYLE_NAME
...
def apply_selected_style(style_name, prompt_substitute="person"):
    ...
    replacement = _resolve_style_replacement(prompt_substitute)
    return style_prompt.replace("{prompt}", replacement).strip(), style_neg_prompt, "(No style)"
...
def main(pretrained_model_name_or_path=DEFAULT_MODEL):

    def _build_pipe_from_checkpoint(model_name, PipeClass):
    ...
    def _build_pipe_from_pretrained(model_name, PipeClass):
        p = PipeClass.from_pretrained(
            model_name,
            controlnet=[controlnet_identitynet],
            torch_dtype=dtype,
            feature_extractor=None,
        ).to(device)
        p.scheduler = diffusers.DPMSolverMultistepScheduler.from_config(p.scheduler.config)
        return p

    def _is_file_model(name):
        return name.endswith((".ckpt", ".safetensors"))

    if vram_gb >= 15:
        pipe = None
    else:
        PipeClass = StableDiffusionXLInstantIDPipeline
        if _is_file_model(pretrained_model_name_or_path):
            pipe = _build_pipe_from_checkpoint(pretrained_model_name_or_path, PipeClass)
        else:
            pipe = _build_pipe_from_pretrained(pretrained_model_name_or_path, PipeClass)

    print(
        f"Detected GPU VRAM: {vram_gb:.2f} GB → "
        f"VAE Tiling: {'Enabled' if default_vae_tiling else 'Disabled'} "
        f"(you can change it manually in the UI)"
    )
    ...
    def load_and_cache_controlnet_model(controlnet_type):
        if controlnet_type not in cached_controlnet_models:
            ...
            model = ControlNetModel.from_pretrained(
                controlnet_model_paths[controlnet_type], torch_dtype=dtype
            ).to(device)
    ...
    def load_model_and_update_pipe(model_name, enable_img2img):
        ...
        if _is_file_model(model_name):
            pipe = _build_pipe_from_checkpoint(model_name, PipeClass)
        else:
            pipe = _build_pipe_from_pretrained(model_name, PipeClass)
        pipe.load_ip_adapter_instantid(face_adapter)
        if vram_gb >= 15:
            pipe._current_model = model_name
        return pipe

    def _get_lora_slots(*args):
        slots = []
        for idx in range(NUM_LORAS):
            sel = args[idx * 3]
            dis = args[idx * 3 + 1]
            sc = args[idx * 3 + 2]
            slots.append((sel, dis, sc, idx + 1))
        return slots

    def _load_loras(pipe, lora_slots):
        pipe.unload_lora_weights()
        loras_to_load = []
        for selection, disabled, scale, idx in lora_slots:
            if selection and not disabled:
                lora_path = os.path.join(LORAS_DIR, selection)
        ...
        if loras_to_load:
            ...
            for i, lora_item in enumerate(loras_to_load):
                sanitized = lora_item['name'].replace('.safetensors', '').replace('.', '_')
                adapter_name = f"lora_{i}_{sanitized}"
                pipe.load_lora_weights(LORAS_DIR, weight_name=lora_item["name"], adapter_name=adapter_name)
    ...
    def _build_lora_info_str(enable_lora, lora_slots):
        if not enable_lora:
            return "Disabled"
        details = []
        for selection, disabled, scale, idx in lora_slots:
            if selection:
                path = os.path.join(LORAS_DIR, selection)
                if not disabled and os.path.exists(path):
                    details.append(f"LoRA {idx}: {selection} (Scale: {scale})")
                elif disabled:
                    details.append(f"LoRA {idx}: Manually disabled")
                else:
                    details.append(f"LoRA {idx}: {selection} (Not found)")
        return "; ".join(details) if details else "Disabled"

    def _lora_metadata_lines(enable_lora, lora_slots):
        lines = []
        for selection, disabled, scale, idx in lora_slots:
            exists = selection and os.path.exists(os.path.join(LORAS_DIR, selection))
            active = enable_lora and exists and not disabled
            lines.append(f"LoRA {idx} selection: {selection if active else 'None'}")
            lines.append(f"LoRA {idx} scale: {scale if active else 'Disabled'}")
        return "\n".join(lines)
    ...
    def generate_image(
        ...
        scheduler,
        enable_lora,
        disable_lora_1, lora_scale, lora_selection,
        disable_lora_2, lora_scale_2, lora_selection_2,
        disable_lora_3, lora_scale_3, lora_selection_3,
        disable_lora_4, lora_scale_4, lora_selection_4,
        disable_lora_5, lora_scale_5, lora_selection_5,
        disable_lora_6, lora_scale_6, lora_selection_6,
        disable_lora_7, lora_scale_7, lora_selection_7,
        disable_lora_8, lora_scale_8, lora_selection_8,
        enhance_face_region,
        enhance_strength,
        ...
        file_prefix = (
            DEFAULT_FILE_PREFIX if not file_prefix
            else (f"{file_prefix}_" if not file_prefix.endswith('_') else file_prefix)
        )
        ...
        custom_size = (int(custom_resize_width), int(custom_resize_height)) if enable_custom_resize else None
        ...
        def _resize(img):
            return resize_img(
                ...
                pad_to_max_side=pad_to_max_side,
                exact_ratio=exact_ratio,
            )

        face_image = _resize(load_image(face_image_path))
        ...
        if pose_image_path is not None:
            pose_image = _resize(load_image(pose_image_path))
        ...
        if enhance_face_region:
            ...
            padding_ratio = {
                "Balanced": 0.15,
                "High": 0.3,
                "Custom": custom_enhance_padding,
            }.get(enhance_strength, 0.0)
        ...
        if len(controlnet_selection) > 0:
            ...
            for k in list(cached_controlnet_models.keys()):
                if k not in controlnet_selection:
                    ...
                    torch.cuda.empty_cache()
                    gc.collect()
            controlnet_scales = {"pose": pose_strength, "canny": canny_strength, "depth": depth_strength}
            controlnet_models_to_use = []
            controlnet_images = []
            for s in controlnet_selection:
                controlnet_models_to_use.append(load_and_cache_controlnet_model(s))
        ...
        print(f"Input face image: {face_image_filename}")
        print(f"Reference pose image: {pose_image_filename}")
        print(f"Steps: {num_steps}")
        print(f"img2img Mode: {'Enabled' if enable_img2img else 'Disabled'}")
        if enable_img2img:
            print(f"img2img Denoising Strength: {strength}")
        print(
            f"Enhance non-face region: {'True' if enhance_face_region else 'False'} "
            f"({enhance_strength}{f' | Padding: {custom_enhance_padding:.2f}' if enhance_strength == 'Custom' else ''})"
        )
        ...
        print(
            f"ControlNet selection: {controlnet_selection} | "
            f"Strengths - Pose: {pose_strength}, Canny: {canny_strength}, Depth: {depth_strength}"
        )
        print(f"IdentityNet strength: {identitynet_strength_ratio}")
        print(f"Adapter strength: {adapter_strength_ratio}")
        print(f"LoRA(s): {_build_lora_info_str(enable_lora, lora_slots)}")
        ...
        for i in range(num_outputs):
            ...
            if is_slow_scheduler:
                def gradio_callback_lambda(pipe_obj, step, timestep, callback_kwargs, _i=i, _steps=steps, _tracker=step_tracker):
                    if step != _tracker["last"]:
                        _tracker["last"] = step
                        _tracker["total"] += 1
                    est_total = _steps * 2
                    progress(
                        ((_i / num_outputs) + (_tracker["total"] / est_total) / num_outputs),
                        desc=f"Generating image {_i + 1} of {num_outputs} "
                             f"(Step {min(_tracker['total'] // 2, _steps)}/{_steps})"
                    )
                    return callback_kwargs
            else:
                def gradio_callback_lambda(pipe_obj, step, timestep, callback_kwargs, _i=i, _steps=steps):
                    progress(
                        ((_i / num_outputs) + (((step + 1) / _steps) / num_outputs)),
                        desc=f"Generating image {_i + 1} of {num_outputs} (Step {step + 1}/{_steps})"
                    )
                    return callback_kwargs
            ...
            if enable_img2img:
                result = pipe(**common_kwargs, image=face_image, control_image=control_images, strength=strength)
            else:
                result = pipe(**common_kwargs, image=control_images, control_mask=control_mask)
            ...
            info_text = (
                ...
                f"Input Face Image: {face_image_filename}\n"
                f"Reference Pose Image: {pose_image_filename}\n"
                f"Detection size: {current_det_size}\n"
                f"Steps: {num_steps}\n"
                f"Guidance scale: {guidance_scale}\n"
                f"Seed: {seed + i}\n"
                f"Model: {model_name}\n"
                f"ControlNet selection: {controlnet_selection}\n"
                f"Max resize side: {resize_max_side}\n"
                f"Image size: {width}x{height}\n"
                f"Exact aspect ratio: {exact_ratio}\n"
                f"Enhance non-face region: {enhance_face_region}\n"
                f"Enhance region profile: {enhance_strength}\n"
                f"Enhance padding ratio: {custom_enhance_padding}\n"
                f"Resize mode: {resize_mode}\n"
                f"Pad to max side: {pad_to_max_side}\n"
                f"Use custom resize: {enable_custom_resize}\n"
                f"Custom resize size: {custom_resize_width}x{custom_resize_height}\n"
                f"img2img Strength: {strength}\n"
                f"img2img Mode Enabled: {enable_img2img}\n"
                f"IdentityNet strength: {identitynet_strength_ratio}\n"
                f"Adapter strength: {adapter_strength_ratio}\n"
                f"Pose strength: {pose_strength}\n"
                f"Canny strength: {canny_strength}\n"
                f"Depth strength: {depth_strength}\n"
                f"LoRA Enabled: {enable_lora}\n"
                + _lora_metadata_lines(enable_lora, lora_slots) + "\n"
    ...
    _LORA_DEFAULT_SCALES = [1.0, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]
    _LORA_ORDINALS = ["first", "second", "third", "fourth", "fifth", "sixth", "seventh", "eighth"]

    def _build_lora_row(n):
        ordinal = _LORA_ORDINALS[n - 1]
        default_scale = _LORA_DEFAULT_SCALES[n - 1]
        with gr.Row(visible=False) as row:
            dropdown = gr.Dropdown(
                label=f"Select LoRA {n}",
                choices=[""] + get_available_loras(),
                value=None,
                allow_custom_value=True,
                info=f"Select the {ordinal} LoRA.",
                scale=3,
            )
            slider = gr.Slider(
                label=f"LoRA {n} Scale",
                minimum=0.0, maximum=2.0, step=0.05,
                value=default_scale,
                info=f"Strength of the {ordinal} LoRA effect.",
                scale=3,
            )
            disable_cb = gr.Checkbox(label=f"Disable LoRA {n}", value=False, scale=1)
        return row, dropdown, slider, disable_cb
    ...
    with gr.Blocks() as gui:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                with gr.Row():
                    scheduler = gr.Dropdown(choices=SCHEDULERS, value="DPMSolverMultistepScheduler", container=False)
            ...
            refresh_models.click(fn=refresh_model_list, outputs=model_name)
        ...
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    ...
                    pose_file = gr.Image(
                        label="Reference pose image (Optional)", height=400, type="filepath"
                    )
                ...
                with gr.Accordion("⚙️ Style templates and other settings including custom resolution", open=False) as style_settings_accordion:
                    with gr.Group():
                        ...
                        style.change(fn=on_style_change, inputs=style, outputs=[prompt, negative_prompt])

                    feeling_lucky_btn = gr.Button(
                        "🎰 Insert a random style from the style templates into prompt & negative prompt fields.",
                    ...
                    prompt_replacement.change(fn=lambda _: None, inputs=prompt_replacement, outputs=[], queue=False)
                    ...
                    with gr.Accordion("Negative Prompt Profiles", open=False):
                        with gr.Group():
                            ...
                            apply_negative_profile_btn = gr.Button(
                                "Apply selected negative prompt profile", size="sm", min_width=200
                            )
                    ...
                    with gr.Row():
                        ...
                        open_folder_btn_settings = gr.Button("📁", min_width=60, scale=0)
                        open_folder_btn_settings.click(fn=open_output_folder, inputs=[], outputs=[], queue=False)
                    ...
                    with gr.Row():
                        resize_mode_dropdown = gr.Dropdown(
                            ...
                            choices=["LANCZOS", "BILINEAR", "HAMMING", "BICUBIC", "BOX", "NEAREST"],
                            value="LANCZOS",
                        )
                    with gr.Row():
                        exact_ratio = gr.Checkbox(
                            label="Maintain the exact aspect ratio of input photos.", value=True
                        )
                        pad_to_max_checkbox = gr.Checkbox(
                            label="Square padding (keeps subject proportions intact)", value=False
                        )
                ...
                enable_custom_resize.change(
                    ...
                    outputs=[custom_resize_width, custom_resize_height, resize_max_side_slider, pad_to_max_checkbox, exact_ratio],
                )

                def toggle_resize_step(exact_ratio_val):
                    return gr.update(step=32 if exact_ratio_val else 64)

                exact_ratio.change(fn=toggle_resize_step, inputs=exact_ratio, outputs=[resize_max_side_slider])

                with gr.Row():
                    generate = gr.Button("Generate", scale=8, variant="primary")
                    num_outputs = gr.Number(
                        value=1, step=1, minimum=1,
                        scale=0, min_width=70, container=False, show_label=False,
                    )
                    open_folder_btn_main = gr.Button("📁", min_width=60, scale=0)
                    open_folder_btn_main.click(fn=open_output_folder, inputs=[], outputs=[], queue=False)

                identitynet_strength_ratio = gr.Slider(
                    label="IdentityNet strength (weight of face fidelity retention from the input photo)",
                    minimum=0, maximum=1.5, step=0.05, value=0.7,
                )
                adapter_strength_ratio = gr.Slider(
                    label="Image adapter strength (weight of detail retention from the input photo)",
                    minimum=0, maximum=1.5, step=0.05, value=0.6,
                )
                ...
                with gr.Accordion("Controlnet", open=False) as controlnet_accordion:
                    ...
                    pose_strength  = gr.Slider(label="Pose strength",  minimum=0, maximum=1.5, step=0.05, value=0.40)
                    canny_strength = gr.Slider(label="Canny strength", minimum=0, maximum=1.5, step=0.05, value=0.40)
                    depth_strength = gr.Slider(label="Depth strength", minimum=0, maximum=1.5, step=0.05, value=0.40)

                with gr.Row():
                    guidance_scale = gr.Slider(label="Guidance scale (CFG)", minimum=1.0, maximum=20.0, step=0.1, value=4)
                    num_steps = gr.Slider(label="Sampling steps", minimum=1, maximum=150, step=1, value=22)

                with gr.Row():
                    randomize_seed = gr.Checkbox(label="Randomize seed", scale=1, value=True)
                    seed = gr.Number(minimum=0, maximum=MAX_SEED, step=1, scale=4, value=12345, show_label=False)
                ...
                with gr.Row():
                    ...
                    custom_enhance_padding = gr.Slider(
                        ...
                        minimum=0.0, maximum=0.9, step=0.05, value=0.15,
                        visible=False, scale=3, interactive=True,
                    )
                    enhance_strength.change(
                        fn=lambda v: gr.update(visible=(v == "Custom")),
                        inputs=enhance_strength, outputs=custom_enhance_padding,
                    )
            ...
            with gr.Column(scale=1):
                gallery = gr.Gallery(
                    label="Generated image(s) preview. Open the output folder for full view.",
                    height=400, object_fit="contain",
                )
                with gr.Row():
                    generate_alt = gr.Button("Generate (Extra Right Side Button)", variant="primary")
                    open_folder_btn_right = gr.Button("📁", min_width=60, scale=0)
                    open_folder_btn_right.click(fn=open_output_folder, inputs=[], outputs=[], queue=False)

                with gr.Row():
                    enable_img2img = gr.Checkbox(
                        label="Enable img2img mode", value=False,
                    ...
                    strength = gr.Slider(
                        ...
                        visible=False, scale=5,
                        info="Use this for more control over e.g., location setting, clothing style, pose, etc. A lower value preserves more of the original image. CFG scale of ~3 is recommended.",
                    )
                enable_img2img.change(
                    fn=lambda enable: gr.update(visible=enable),
                    inputs=enable_img2img, outputs=strength,
                )
                ...
                with gr.Accordion("PNG Metadata Reader", open=True):
                    with gr.Row():
                        metadata_input = gr.Image(
                            ...
                            type="filepath", height=500, width=500,
                        )
                        metadata_output = gr.Textbox(
                            label="Generation Metadata", interactive=False, lines=22, max_lines=22
                        )
                    with gr.Row():
                    ...
                    metadata_input.upload(
                        ...
                        inputs=metadata_input, outputs=[metadata_input, metadata_output],
                    )

                with gr.Column():
                    enable_lora = gr.Checkbox(
                        label=r"Enable LoRA(s) from your Models\Loras folder", value=False
                    )
                    lora_info = gr.Markdown(
                        "Up to eight LoRAs can be loaded. Only SDXL and Pony LoRAs supported. "
                        "The 'Disable Lora' checkbox is only needed if you have a Lora selected.",
                        visible=False,
                    )
                    enable_lora.change(
                        fn=lambda x: gr.Markdown(visible=x),
                        inputs=enable_lora, outputs=lora_info,
                    )
                    with gr.Row():
                    ...
                    lora_rows, lora_dropdowns, lora_scales, lora_disables = [], [], [], []
                    for n in range(1, NUM_LORAS + 1):
                        row, dd, sl, dis = _build_lora_row(n)
                        lora_rows.append(row)
                        lora_dropdowns.append(dd)
                        lora_scales.append(sl)
                        lora_disables.append(dis)

                    (
                        lora_row_1, lora_row_2, lora_row_3, lora_row_4,
                        lora_row_5, lora_row_6, lora_row_7, lora_row_8,
                    ) = lora_rows
                    (
                    ...
                    (
                        lora_scale,   lora_scale_2, lora_scale_3, lora_scale_4,
                        lora_scale_5, lora_scale_6, lora_scale_7, lora_scale_8,
                    ) = lora_scales
                    (
                    ...
                    for dis_cb, dd, sl in zip(lora_disables, lora_dropdowns, lora_scales):
                        dis_cb.change(
                            fn=lambda x: [gr.update(interactive=not x), gr.update(interactive=not x)],
                            inputs=dis_cb, outputs=[dd, sl], queue=False,
                        )

                    def refresh_lora_list():
                        loras = [""] + get_available_loras()
                        return [gr.update(choices=loras)] * NUM_LORAS

                    refresh_loras.click(fn=refresh_lora_list, outputs=lora_dropdowns)

                    def clear_lora_list():
                        return (
                            [gr.update(value=None)] * NUM_LORAS
                            + [gr.update(value=False)] * NUM_LORAS
                        )

                    clear_loras.click(
                        fn=clear_lora_list,
                        outputs=lora_dropdowns + lora_disables,
                    )

                shared_inputs = [
                    resize_max_side_slider, face_file, pose_file,
                    prompt, negative_prompt, style, prompt_replacement,
                    num_steps, identitynet_strength_ratio, adapter_strength_ratio,
                    pose_strength, canny_strength, depth_strength,
                    controlnet_selection, guidance_scale, seed, scheduler,
                    enable_lora,
                    disable_lora_1, lora_scale,   lora_selection,
                    disable_lora_2, lora_scale_2,  lora_selection_2,
                    disable_lora_3, lora_scale_3,  lora_selection_3,
                    ...
                    disable_lora_7, lora_scale_7,  lora_selection_7,
                    disable_lora_8, lora_scale_8,  lora_selection_8,
                    enhance_face_region, enhance_strength, custom_enhance_padding,
                    num_outputs, model_name, det_size_name, file_prefix,
                    enable_vae_tiling, resize_mode_dropdown, pad_to_max_checkbox,
                    enable_custom_resize, custom_resize_width, custom_resize_height,
                    enable_img2img, strength, exact_ratio,
                ]

                for btn in [generate, generate_alt, generate_alt_2]:
                    btn.click(
                        fn=randomize_seed_fn, inputs=[seed, randomize_seed], outputs=seed,
                        queue=False, api_name=False,
                    ).then(fn=generate_image, inputs=shared_inputs, outputs=[gallery])

                LORA_OUTPUTS = []
                for row, dd, sl in zip(lora_rows, lora_dropdowns, lora_scales):
                    LORA_OUTPUTS.extend([row, dd, sl])
                LORA_OUTPUTS.extend([refresh_loras, clear_loras])

                enable_lora.input(
                    fn=toggle_lora_ui, inputs=[enable_lora], outputs=LORA_OUTPUTS, queue=False
                )

                def extract_all_settings(metadata_text):
                    accordion_update = gr.update(open=False)
                    settings = {
                        "prompt": "", "negative_prompt": DEFAULT_NEGATIVE_PROFILE,
                        "resize_max_side": 1280, "seed": 12345, "num_steps": 22,
                        "guidance_scale": 4.0, "enable_img2img": False, "strength": 0.95,
                        "identitynet_strength_ratio": 0.7, "adapter_strength_ratio": 0.6,
                        "pose_strength": 0.40, "canny_strength": 0.40, "depth_strength": 0.40,
                        "scheduler": "DPMSolverMultistepScheduler", "exact_ratio": True,
                        "enable_lora": False,
                        "enhance_face_region": True, "enhance_strength": "Balanced",
                        "custom_enhance_padding": 0.15, "style": DEFAULT_STYLE_NAME,
                        "randomize_seed": True, "controlnet_selection": [],
                        "model_name": DEFAULT_MODEL, "det_size_name": "640x640 (default)",
                        "resize_mode": "LANCZOS", "pad_to_max_side": False,
                        "enable_custom_resize": False,
                        "custom_resize_width": 960, "custom_resize_height": 1280,
                    }
                    for n in range(1, NUM_LORAS + 1):
                        settings[f"lora_selection_{n}" if n > 1 else "lora_selection"] = None
                        settings[f"lora_scale_{n}" if n > 1 else "lora_scale"] = _LORA_DEFAULT_SCALES[n - 1]
                        settings[f"disable_lora_{n}"] = False
                    ...
                    if metadata_text:
                        ...
                        for idx, line in enumerate(lines):
                            ...
                            if stripped.startswith("Prompt:"):
                                val = line[len("Prompt:"):].lstrip(" ")
                                prompt_lines = [val]
                                ci = idx + 1
                                while ci < len(lines):
                                    if lines[ci].strip().startswith("Negative Prompt:"):
                                        break
                                    prompt_lines.append(lines[ci])
                                    ci += 1
                                settings["prompt"] = "\n".join(prompt_lines)

                            elif stripped.startswith("Negative Prompt:"):
                                val = line[len("Negative Prompt:"):].lstrip(" ")
                                neg_lines = [val]
                                ci = idx + 1
                                while ci < len(lines):
                                    if lines[ci].strip().startswith(("Input Face Image:", "Detection size:")):
                                        break
                                    neg_lines.append(lines[ci])
                                    ci += 1
                                ...
                                sched_text = line.replace("Scheduler:", "").strip()
                                sched_name = (
                                    sched_text.split(".")[-1].replace("'>", "")
                                    if "scheduling_" in sched_text else sched_text
                                )
                                if sched_name in SCHEDULERS:
                                    settings["scheduler"] = sched_name
                            elif line.startswith("ControlNet selection:"):
                                cn_sel = line.replace("ControlNet selection:", "").strip()
                                if cn_sel.startswith("["):
                                    try:
                                        cn_list = eval(cn_sel)
                                        if isinstance(cn_list, list):
                                            clean_list = [x.strip("'\" ") for x in cn_list]
                                            settings["controlnet_selection"] = clean_list
                                            if set(clean_list) & {"pose", "canny", "depth"}:
                                                accordion_update = gr.update(open=True)
                                    except Exception:
                                        pass
                            elif line.startswith("Model:"):
                                m = line.replace("Model:", "").strip()
                                if m in AVAILABLE_MODELS:
                                    settings["model_name"] = m
                                else:
                                    gr.Warning(f"Model '{m}' used for this image can't be found in your models folder. Falling back to the default model.")
                                ...
                                for n in range(1, NUM_LORAS + 1):
                                    sel_key = f"lora_selection_{n}" if n > 1 else "lora_selection"
                                    sc_key  = f"lora_scale_{n}" if n > 1 else "lora_scale"
                                    dis_key = f"disable_lora_{n}"
                                    if line.startswith(f"LoRA {n} selection:"):
                                        val = line.replace(f"LoRA {n} selection:", "").strip()
                                        settings[sel_key] = val if val != "None" else None
                                    elif line.startswith(f"LoRA {n} scale:"):
                                        val = line.replace(f"LoRA {n} scale:", "").strip()
                                        if val != "Disabled":
                                            try:
                                                settings[sc_key] = float(val)
                                            except Exception:
                                                pass

                    open_settings_accordion = (
                        settings["enable_custom_resize"]
                        or settings["pad_to_max_side"]
                        or not settings["exact_ratio"]
                    )
                ...
                apply_metadata_btn.click(
                    ...
                    outputs=[
                        ...
                        num_steps, enable_img2img, strength,
                        identitynet_strength_ratio, adapter_strength_ratio,
                        pose_strength, canny_strength, depth_strength,
                        guidance_scale, seed, scheduler,
                        enable_lora, enhance_face_region, enhance_strength, custom_enhance_padding,
                        lora_scale,   lora_selection,
                        lora_scale_2, lora_selection_2,
                        lora_scale_3, lora_selection_3,
                        lora_scale_4, lora_selection_4,
                        lora_scale_5, lora_selection_5,
                        lora_scale_6, lora_selection_6,
                        lora_scale_7, lora_selection_7,
                        lora_scale_8, lora_selection_8,
                        randomize_seed, controlnet_selection,
                        model_name, det_size_name, resize_max_side_slider,
                        disable_lora_1, disable_lora_2, disable_lora_3, disable_lora_4,
                        disable_lora_5, disable_lora_6, disable_lora_7, disable_lora_8,
                        resize_mode_dropdown, pad_to_max_checkbox,
                        enable_custom_resize, custom_resize_width, custom_resize_height,
                        exact_ratio, controlnet_accordion, style_settings_accordion,
                    ],
                ).then(fn=toggle_lora_ui, inputs=[enable_lora], outputs=LORA_OUTPUTS)

        with gr.Accordion("📝 Click to show/hide usage tips", open=False):
            gr.Markdown(article)
        gr.Markdown(
            "<b>InstantID: Unlocked v5.4.0</b> - "
            "<a href='https://github.com/eniora/InstantID-Unlocked' target='_blank'>"
            "<b>Github fork page for InstantID: Unlocked</b></a><br>"
        )
